...
# Define the custom L1 Distance layer used in the model
class L1Dist(Layer):
...
# Function to add a red rectangle in the center of the image
...
# Function to zoom out the image to show a larger area
...
# Function to update the stream with a rectangle overlay indicating the capture area
...
# Function to verify if the person in the captured image matches with any verification images
def verify(image, model, detection_threshold=0.99, verification_threshold=0.8):
    ...
    return "Verified" if verification > verification_threshold else "Unverified"

# Function called when the capture button is pressed to save and verify the image
def capture_and_save(image):
    ...
    verification_status = verify(image_bgr, model)
